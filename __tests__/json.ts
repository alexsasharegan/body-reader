...
describe("Parser.Json()", () => {
  ...
  describe("with limit option", function() {
    it("should 413 when over limit with Content-Length", function(done) {
      let body = JSON.stringify(new Array(1024).fill("1"));
      request(createServer({ limit: Bytes.toBytes(1, "KiB") }))
        .post("/")
        .set("Content-Type", "application/json")
        .set("Content-Length", Buffer.from(body).byteLength)
        .set("X-Request-Id", "1")
        .send(body)
        .expect(413, done);
    });
    it('should error with type = "request entity too large"', function(done) {
    ...
    //   let buf = Buffer.alloc(1024, ".");
    //   let server = createServer({ limit: Bytes.toBytes(1, "KiB") });
    //   let test = request(server).post("/");
    //   test.set("Content-Type", "application/json");
    //   test.set("Transfer-Encoding", "chunked");
    //   test.set("X-Request-Id", "3");
    //   test.write('{"str":');
    ...
    //   let buf = Buffer.alloc(Bytes.toBytes(2, "KiB"), ".");
    //   let options = { limit: Bytes.toBytes(1, "KiB") };
    //   let server = createServer(options);
    //   options.limit = Bytes.toBytes(100, "KiB");
    //   request(server)
    //     .post("/")
    //     .set("Content-Type", "application/json")
    //     .send(JSON.stringify({ str: buf.toString() }))
    ...
    //   let server = createServer({ limit: Bytes.toBytes(8, "KiB") });
    //   let test = request(server).post("/");
    //   test.set("Content-Type", "application/json");
    //   test.write(buf);
    //   test.write(buf);
  ...
  //       let test = request(this.server).post("/");
  ...
  //       let server = createServer({ type: accept });
  //       function accept(req) {
